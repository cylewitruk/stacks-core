...
## Building from stacks-core

From the repository root, build the benchmark binary with:

```bash
cargo build --release -p stacks-bench
```

The executable is `target/release/stacks-bench`. For a direct-attached benchmark
snapshot, `--source` must name the mounted node-data directory containing
`chainstate/`. `--dangerous-no-chainstate-copy` writes to that snapshot directly;
use a disposable or host-snapshotted disk for each run. In range mode, `--count`
includes warmup blocks: `--count 25000 --warmup 5000` measures 20,000 blocks.

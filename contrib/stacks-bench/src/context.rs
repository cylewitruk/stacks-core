...
impl<'a> BenchContext<'a> {
    ...
    pub fn open_stacks_chainstate(&self) -> Result<(StacksChainState, Burnchain)> {
        ...
        let result_capacity = env::var("MARF_BENCH_RESULT_CACHE")
            .unwrap_or_else(|_| "2048".into())
            .parse::<usize>()?;
        let patch_capacity = env::var("MARF_BENCH_PATCH_CACHE")
            .unwrap_or_else(|_| "16".into())
            .parse::<usize>()?;
        let root_cache = env::var("MARF_BENCH_ROOT_CACHE").as_deref() == Ok("1");
        burnchain
            .marf_opts
            .get_or_insert_with(MARFOpenOpts::default)
            .root_node_cache = root_cache;
        burnchain
            .marf_opts
            .as_mut()
            .unwrap()
            .resolved_patch_cache_capacity = patch_capacity;
        burnchain.marf_opts.as_mut().unwrap().result_cache_capacity = result_capacity;
        ...
        marf_opts.root_node_cache = root_cache;
        marf_opts.resolved_patch_cache_capacity = patch_capacity;
        marf_opts.result_cache_capacity = result_capacity;

...
mod tests {
    ...
    use super::BaselineCalibrationResult;
    use crate::wire::CommandResult;

    /// Keeps the host runner's calibration-ID JSON path stable.
    #[test]
    fn calibration_id_is_in_result_envelope() {
        let calibration = BaselineCalibrationResult {
            calibration_id: Some(17),
            chainstate_id: 3,
            start_parent_index_hash: "00".repeat(32),
            interrupted: false,
            calibration: None,
        };
        let envelope = CommandResult::from_payload(&calibration, 0.0).unwrap();
        let json = serde_json::to_value(envelope).unwrap();
        assert_eq!(
            json.pointer("/result/calibration_id")
                .and_then(|v| v.as_i64()),
            Some(17)
        );
        assert_eq!(
            json.pointer("/result_type").and_then(|v| v.as_str()),
            Some("baseline_calibration")
        );
    }
}

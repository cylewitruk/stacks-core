...
//!
//! The `#[tool_router]` impl centralises tool declarations. Each tool's
//! handler, query logic, and DTOs live in their own submodule.
...
impl StacksBenchServer {
    /// Build the tool router. Wraps the macro-generated private
    /// `tool_router()` so it can be called from [`StacksBenchServer::new`].

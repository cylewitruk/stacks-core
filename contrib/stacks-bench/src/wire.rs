...
/// Opaque passthrough payload. The `metabase` / `explorer` commands return a
/// bare `serde_json::Value` status blob — they are UI launchers, not stable
/// data contracts — so they are tagged `"opaque"` / version `0` and are not
/// covered by the wire-evolution policy.

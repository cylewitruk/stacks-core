//! Bounded planning and durable checkpoints for a physical-only V1 to V2 rewrite.
use std::error::Error;
...
const CODEC: &str = "compact-raw-v2-production-plan-1";
...
fn source(path: &Path) -> Result<Connection> {
    ...
    if NodeRecordFormat::from_database(&db)? != NodeRecordFormat::TypeFirstV1 {
        return Err("source must use TypeFirstV1".into());
    }
    Ok(db)
}
    ...
    let result = ordered_pipeline::run(
        ...
        |blob| {
            let run = || -> Result<_> {
                ...
                let plan = BlobRelocation::plan_format(
                    &bytes,
        ...
        |(id, external, source_length, length, offsets, digest)| {
...
fn destination(config: &Config, directory: &Path, binding: &str) -> Result<Connection> {
    ...
    if !directory.exists() {
        ...
        db.execute_batch("UPDATE marf_record_format SET version=-2; INSERT INTO format_migration_state VALUES('completed','0'),('end','0'); COMMIT")?;
    }
...
fn rewrite_with_event(
    ...
    NodeRecordFormat::TypeFirstV2.publish(&db)?;
...
mod tests {
    ...
    fn fixture(directory: &Path, external: bool, blocks: u32) -> Config {
        ...
        NodeRecordFormat::TypeFirstV1
            .publish(marf.sqlite_conn())
            .unwrap();
        marf.set_record_format(NodeRecordFormat::TypeFirstV1);
        ...
        for height in 1..=blocks {
            ...
            extent.extent = Some(ValueExtent {
                store_id: [4; 16],
                offset: 48,
                length: 100,
            });

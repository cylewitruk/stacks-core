...
This first prototype does not implement online delta compaction or index-generation replacement. Its absolute generation path must remain available alongside any cloned database. Do not remove a published generation while a database references it. Backup/restore, portable serialization, corruption recovery and bounded online rebuilding remain production integration work.
